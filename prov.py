...
# valor_mercado = 250
# prob_arista = 0.07
# cntd_mosq_por_lugar = 40
# prob_picar = 0.02
# prob_infectarse = 0.15
# prob_morir_por_hora = 0.000001
# #TODO: Volver a simular con los mismo parametros, ahora teniendo en cuenta que las personas se recuperan.
...
# infected_people = [0 for x in range(int(len(pers_hour)/24) + 1)]
# healthy_people = [0 for x in range(int(len(pers_hour)/24) + 1)]
# dead_people = [0 for x in range(int(len(pers_hour)/24) + 1)]
# pers_counted_i = []
# pers_counted_h = []
# pers_counted_d = []
# count = -1
